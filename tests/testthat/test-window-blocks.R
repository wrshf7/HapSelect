...
test_that("def_blocks_window returns a named list with one entry per chromosome", {
  map <- make_window_map_fixture()

  blocks <- def_blocks_window(map, window = 3, method = "window_snp")
...
test_that("def_blocks_window output is a partition of the map", {
  map <- make_window_map_fixture()

  for (args in list(list(window = 3,   method = "window_snp"),
                    list(window = 200, method = "window_map"))) {

    blocks <- def_blocks_window(map, window = args$window, method = args$method)
...
test_that("window_snp = 1 makes every marker a singleton block", {
  ...
  blocks <- def_blocks_window(map, window = 1, method = "window_snp")
...
test_that("window_snp larger than the chromosome gives a single block", {
  ...
  blocks <- def_blocks_window(map, window = 100, method = "window_snp")
...
test_that("window_map assigns markers to a fixed grid anchored at 0", {
  ...
  blocks <- def_blocks_window(map, window = 200, method = "window_map")
...
test_that("window_map accepts a fractional window for cM maps", {
  ...
  blocks <- def_blocks_window(map, window = 0.5, method = "window_map")
...
test_that("def_blocks_window warns and sorts a map that is not ordered by position", {
  map <- make_window_map_fixture()
  ...
  for (reordered in list(map[c(7, 2, 9, 4, 1, 10, 5, 3, 8, 6), ],
    ...
    expect_warning(snp <- def_blocks_window(reordered, window = 3, method = "window_snp"),
                   "not sorted by Position")
    expect_warning(mp  <- def_blocks_window(reordered, window = 200, method = "window_map"),
                   "not sorted by Position")

    # repaired output must match what the already-sorted map produces
    expect_equal(snp, def_blocks_window(map, window = 3,   method = "window_snp"))
    expect_equal(mp,  def_blocks_window(map, window = 200, method = "window_map"))
  }

  # the warning names the offending marker and points at the fix
  expect_warning(def_blocks_window(map[c(2, 1, 3:10), ], window = 3, method = "window_snp"),
                 "'c1'")
  expect_warning(def_blocks_window(map[c(2, 1, 3:10), ], window = 3, method = "window_snp"),
                 "order_map")
  ...
  suppressWarnings(
    blocks <- def_blocks_window(map[nrow(map):1, ], window = 200, method = "window_map"))
...
test_that("column order in the map does not matter", {
  ...
  expect_warning(blocks <- def_blocks_window(reordered_cols, window = 3,
                                             method = "window_snp"),
                 "not sorted by Position")
  expect_equal(blocks, def_blocks_window(map, window = 3, method = "window_snp"))
})


test_that("def_blocks_window accepts a correctly sorted map", {
  map <- make_window_map_fixture()
  ...
  expect_silent(def_blocks_window(tied, window = 3, method = "window_snp"))

  # chromosomes need not be contiguous, since blocking splits on them anyway
  interleaved <- map[order(map$Position), ]
  expect_silent(def_blocks_window(interleaved, window = 3, method = "window_snp"))

  # a single-marker map has no adjacent pairs to compare
  expect_silent(def_blocks_window(map[1, ], window = 3, method = "window_snp"))

  # a missing position must not be mistaken for an ordering violation
  na_map <- map
  na_map$Position[na_map$SNP == "c3"] <- NA
  expect_warning(def_blocks_window(na_map, window = 3, method = "window_snp"), "dropped")
})


test_that("def_blocks_window requires the order_map() column names", {
  map <- make_window_map_fixture()
  ...
  expect_error(def_blocks_window(alt, window = 3, method = "window_snp"), "order_map")

  no_pos <- map
  colnames(no_pos)[colnames(no_pos) == "Position"] <- "pos"
  expect_error(def_blocks_window(no_pos, window = 3, method = "window_snp"), "order_map")

  chr_txt <- map
  chr_txt$Position <- as.character(chr_txt$Position)
  expect_error(def_blocks_window(chr_txt, window = 3, method = "window_snp"), "numeric")
})


test_that("def_blocks_window drops markers with missing positions and warns", {
  map <- make_window_map_fixture()
...
test_that("def_blocks_window rejects invalid windows and maps", {
  map <- make_window_map_fixture()

  expect_error(def_blocks_window(map, window = 0,    method = "window_snp"), "positive")
  expect_error(def_blocks_window(map, window = -5,   method = "window_map"), "positive")
  expect_error(def_blocks_window(map, window = c(1, 2), method = "window_snp"), "positive")
  expect_error(def_blocks_window(map, window = "10", method = "window_snp"), "positive")

  # a fractional window is meaningless as a marker count, but fine as a distance
  expect_error(def_blocks_window(map, window = 2.5, method = "window_snp"), "whole number")
  expect_silent(def_blocks_window(map, window = 2.5, method = "window_map"))

  expect_error(def_blocks_window(map, window = 3, method = "window_kb"), "arg")

  expect_error(def_blocks_window(map[, c("SNP", "Position")], window = 3), "SNP")
  expect_error(def_blocks_window("not a map", window = 3), "data frame")
})
